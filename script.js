...
function appendValue(value) {
    ...
    // If result was just calculated and user enters a number → start fresh
    ...
    // Count digits only (ignore operators)
    const digitCount = display.innerText.replace(/[^0-9]/g, "").length;

    if (!isNaN(converted) && digitCount >= MAX_DIGITS) {
        return; // restrict to 12 digits
    }
